...
class MUserManager(BaseUserManager):
    def create_user(self, email, password=None, **extra_fields):
        ...
        # WORKAROUND: Database doesn't have AUTO_INCREMENT
        # Manually get next ID from database
        with connection.cursor() as cursor:
            cursor.execute("SELECT COALESCE(MAX(user_id), 0) + 1 FROM m_user")
            next_id = cursor.fetchone()[0]
        
        # Set the ID before save
