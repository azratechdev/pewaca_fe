...
# from your_app.models import Unit, Tagihan, TagihanWarga
...
def report_index_data(request):
    ...
    # Parse periode (YYYY-MM) to filter tagihan
    if periode:
        try:
    ...
    # By Type: Wajib vs Sukarela
    # Join tagihan_warga dengan tagihan untuk ambil tipe
    wajib_total = 0
    sukarela_total = 0
    ...
    # Tunggakan: units yang belum bayar untuk periode ini
    tunggakan_units = tagihan_warga.filter(
        status__in=['unpaid', 'pending', 'overdue']  # sesuaikan status
    ).values('unit_id').distinct().count()
...
def report_cashout_data(request):
    ...
    for tw in sudah_bayar_queryset:
        unit = Unit.objects.get(unit_id=tw.unit_id)
    ...
    for unit in belum_bayar_units:
        # Get expected amount from tagihan
    ...
    # Also check unpaid status in tagihan_warga
...
def report_tunggakan_data(request):
    ...
    if periode:
        # Filter tagihan yang jatuh tempo sampai periode ini
    ...
    for tw in tunggakan_queryset:
        unit_tunggakan_map[tw.unit_id].append(tw)
    
    units_data = []
    total_nominal_all = 0
    
    for unit_id, tunggakan_list in unit_tunggakan_map.items():
        unit_obj = Unit.objects.get(unit_id=unit_id)
        ...
        for tw in tunggakan_list:
            if tw.date_due:
                month_name = tw.date_due.strftime('%B')  # 'April', 'Mei', dll
