...
class UserRegistrationSingleSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        """Check if the email already exists."""
        print("=" * 60)
        print(f"[VALIDATE] Checking email: {value}")
        print("=" * 60)
        if User.objects.filter(email=value).exists():
            print(f"[VALIDATE] ❌ Email already exists: {value}")
            raise serializers.ValidationError("Email sudah terdaftar.")
        print(f"[VALIDATE] ✅ Email valid: {value}")
        return value
    ...
    def create(self, validated_data):
        ...
        code = self.context.get('code')
        print(f"[SERIALIZER] Code from context: {code}")
        print(f"[SERIALIZER] Validated data keys: {validated_data.keys()}")
        print(f"[SERIALIZER] Email: {validated_data.get('email')}")
        print(f"[SERIALIZER] Unit ID: {validated_data.get('unit_id')}")
        
        try:
            with transaction.atomic():
                print("[SERIALIZER] Creating user...")
                user = User.objects.create_user(
                    email=validated_data['email'],
                    username=validated_data['email'],
                    password=validated_data['password'],
                ...
                residence = get_object_or_404(MResidence, code=code)
                print(f"[SERIALIZER] ✅ Residence found: {residence}")

                # Ambil objek referensi berdasarkan ID
                print("[SERIALIZER] Fetching related objects...")
                unit = get_object_or_404(MUnit, unit_id=validated_data['unit_id'])
                print(f"[SERIALIZER] ✅ Unit found: {unit}")
                ...
                education = get_object_or_404(MEducation, id=validated_data['education'])
                family_as = get_object_or_404(MfamilyAs, id=validated_data['family_as'])
                print("[SERIALIZER] ✅ All related objects fetched successfully")
                ...
                print(f"[SERIALIZER] ✅ Warga created successfully! ID: {warga.id if hasattr(warga, 'id') else 'N/A'}")
                print("=" * 60)
                print("[SERIALIZER] Registration completed successfully!")
                print("=" * 60)

                return user

        except IntegrityError as e:
            print(f"[SERIALIZER] ❌ IntegrityError: {str(e)}")
            import traceback
            traceback.print_exc()
            raise serializers.ValidationError(f"Terjadi kesalahan database: {str(e)}")
        except Exception as e:
            print(f"[SERIALIZER] ❌ Exception: {type(e).__name__}")
            print(f"[SERIALIZER] ❌ Message: {str(e)}")
            import traceback
            traceback.print_exc()
