...
- **Nov 11, 2025 - Seller Registration Feature**: Added self-service seller registration allowing any authenticated user (not just pengurus) to become a seller. Features: `/pengurus/seller/register` route with beautiful registration form (reason field, terms checkbox, Warungku green theme), `SellerController::showRegisterForm()` and `processRegistration()` methods with validation and DB transactions. All seller routes (registration, dashboard, store/product/order management) moved outside `role:pengurus` middleware to auth-only level, protected by `is_seller` controller checks instead. This allows residents without pengurus role to register and access seller features.
- **Nov 11, 2025 - Seller Registration CTA Banner**: Relocated seller registration call-to-action from bottom navbar to Warungku marketplace page for better conversion. Added prominent hero banner on `/warungku` with green gradient design (#dcfce7, #bbf7d0 background with #3d7357 border), store icon, "Mulai Jualan di Warungku!" heading, description text, and "Daftar Sekarang" button linking to registration form. Banner displays only for authenticated non-sellers using Session-based auth check (`Session::has('token')` and `Session::get('cred')['is_seller'] == 0`). Note: Application uses custom session-based authentication (not Laravel's default `auth()` facade), storing user credentials in `Session::get('cred')` instead of Laravel's user model. Bottom navbar cleaned up: removed "Daftar Seller" link, kept "Seller" menu only for users with `is_seller=1`. This improves UX by placing CTA where users naturally browse products, increasing seller registration visibility and conversion rate.
- **Nov 11, 2025 - Enhanced Seller Registration with Store Details**: Updated seller registration form to collect complete store information during registration. Form now includes: **Nama Toko** (store name, required, 3-255 chars), **Alamat Toko** (store address, required, 10-500 chars), **Jenis Produk** (product type dropdown with 8 categories: Makanan & Minuman, Sayuran & Buah, Kebutuhan Rumah Tangga, Fashion & Aksesoris, Elektronik, Kesehatan & Kecantikan, Jasa, Lainnya), and checkbox syarat & ketentuan. Database changes: Created migration `2025_11_11_104820_add_user_id_and_product_type_to_stores_table` to add `user_id` (foreign key to users table) and `product_type` columns to stores table. Updated `SellerController::processRegistration()` to automatically create store entry linked to user during registration, with user-provided store name, address, and product type. Store description auto-generated from product type (e.g., "Toko Makanan & Minuman"). **IMPORTANT**: Migration file ready but NOT yet run - needs to be executed on production MySQL server at 43.156.75.206 before deployment. Migration command: `php artisan migrate --database=mysql` on production server.
- **Nov 11, 2025 - Seller Approval Workflow**: Implemented comprehensive approval system for seller registration requests requiring pengurus authorization. **Database**: Created `seller_requests` table (migration `2025_11_11_113136_create_seller_requests_table`) with columns: user_id (FK), store_name, store_address, product_type, status (pending/approved/rejected), rejection_reason, approved_by (FK), approved_at. **Backend**: `SellerRequest` model with status constants and relationships. `SellerRequestController` with lockForUpdate() for race condition prevention, duplicate store validation, Session::has('cred') checks for auth safety, and DB transactions for data integrity. Registration now creates pending seller_request instead of immediate activation. **Frontend**: (1) Pengurus management UI at `/pengurus/seller-requests` with index (list all requests with filters/search), show (detail view with approve/reject buttons), approval/rejection modals with SweetAlert. (2) User status page at `/pengurus/seller/request-status` showing pending/approved/rejected states with appropriate messaging. (3) "Kelola Seller" menu added to `/pengurus` index with pending count badge. (4) Warungku banner hidden for users with pending requests. **Routes**: Added seller request routes in role:pengurus middleware, status route in auth middleware. **Security**: Row locking prevents concurrent approvals, session validation prevents expired auth errors, duplicate store check maintains data integrity. **IMPORTANT**: Migration `2025_11_11_113136_create_seller_requests_table` ready but NOT yet run - needs execution on production MySQL before deployment.
