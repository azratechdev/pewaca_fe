...
- **Nov 11, 2025 - Seller Registration Feature**: Added self-service seller registration allowing any authenticated user (not just pengurus) to become a seller. Features: `/pengurus/seller/register` route with beautiful registration form (reason field, terms checkbox, Warungku green theme), `SellerController::showRegisterForm()` and `processRegistration()` methods with validation and DB transactions, bottom navbar conditionally shows "Daftar Seller" for non-sellers and "Seller" for sellers. All seller routes (registration, dashboard, store/product/order management) moved outside `role:pengurus` middleware to auth-only level, protected by `is_seller` controller checks instead. This allows residents without pengurus role to register and access seller features.
