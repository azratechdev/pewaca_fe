...
- **Nov 11, 2025 - Enhanced Seller Registration with Store Details**: Updated seller registration form to collect complete store information during registration. Form now includes: **Nama Toko** (store name, required, 3-255 chars), **Alamat Toko** (store address, required, 10-500 chars), **Jenis Produk** (product type dropdown with 8 categories: Makanan & Minuman, Sayuran & Buah, Kebutuhan Rumah Tangga, Fashion & Aksesoris, Elektronik, Kesehatan & Kecantikan, Jasa, Lainnya), and **Alasan** (reason, required, 20-500 chars). Database changes: Created migration `2025_11_11_104820_add_user_id_and_product_type_to_stores_table` to add `user_id` (foreign key to users table) and `product_type` columns to stores table. Updated `SellerController::processRegistration()` to automatically create store entry linked to user during registration, with user-provided store name, address, product type, and reason as description. **IMPORTANT**: Migration file ready but NOT yet run - needs to be executed on production MySQL server at 43.156.75.206 before deployment. Migration command: `php artisan migrate --database=mysql` on production server.
