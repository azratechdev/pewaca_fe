...
- **Nov 11, 2025 - Seller Registration CTA Banner**: Relocated seller registration call-to-action from bottom navbar to Warungku marketplace page for better conversion. Added prominent hero banner on `/warungku` with green gradient design (#3d7357, #2d5642), store icon, "Mulai Jualan di Warungku!" heading, description text, and "Daftar Sekarang" button linking to registration form. Banner displays only for authenticated non-sellers (`@auth` and `!$isSeller` conditionals). Bottom navbar cleaned up: removed "Daftar Seller" link, kept "Seller" menu only for users with `is_seller=1`. This improves UX by placing CTA where users naturally browse products, increasing seller registration visibility and conversion rate.
